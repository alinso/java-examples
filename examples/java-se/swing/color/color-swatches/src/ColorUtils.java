...
public class ColorUtils {
	private static final int[] webSafeRGBs = {
	...
	private static final Color[] webSafeColors;

	// A static "initialization block" to create the array of java.awt.Color objects.
	static {
		webSafeColors = new Color[webSafeRGBs.length];

		for (int i = 0; i < webSafeRGBs.length; i++) {
			webSafeColors[i] = new Color(webSafeRGBs[i]);   // Constructor receiving an RGB int value.
		}
	}
	...
	public static Color[] getWebSafeColors() {
		// Clones the array (not the Color objects) because Color is immutable
		// but the array is obviously "mutable". This is not strictly necessary
		// in this Java Example but it is a good practice in general.
		//
		// Note: since Java 5 the cast is not really necessary.
		return (Color[]) webSafeColors.clone();
	}
}